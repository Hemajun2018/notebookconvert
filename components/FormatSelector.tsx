...
export default function FormatSelector({ value, onChange, disabled }: FormatSelectorProps) {
  return (
    <fieldset disabled={disabled} className="w-full">
      <legend className="font-mono text-[11px] uppercase tracking-wider text-ink-500 mb-2">
        Export format
      </legend>
        ...
        {formats.map(fmt => {
          ...
          return (
            <label
              key={fmt.value}
              className={[
                'relative flex flex-col items-center gap-1.5 p-3 sm:p-4 rounded-xl border-2 cursor-pointer transition-all duration-150 select-none',
                isSelected
                  ? 'border-accent-500 bg-accent-50 shadow-sm'
                  : 'border-ink-200 bg-white hover:border-accent-200 hover:bg-accent-50/50',
            ...
            >
              ...
              />
              <svg
                className={`w-6 h-6 ${isSelected ? 'text-accent-600' : 'text-ink-300'}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={1.5}
              ...
              <div className="text-center">
                ...
                <p className="text-[10px] text-ink-500 mt-0.5 leading-tight">{fmt.description}</p>
                <p className="font-mono text-[10px] text-ink-400 mt-0.5">{fmt.extension}</p>
              </div>
              {isSelected && (
